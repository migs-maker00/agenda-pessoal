import { APP_VERSION, hostAtual } from "./config.js?v=2.37.0";
import { fraseFilosoficaDoDia } from "./lib/filosofia.js?v=2.37.0";
import {
  adicionarAviso,
  alternarAvisoFeito,
  avisosDoDia,
  ...
  removerAviso,
  salvarAvisosStorage,
} from "./lib/avisos-agenda.js?v=2.37.0";
import {
  criarHabitoAgua,
  ...
  textoPlanoB,
  todosMicroFeitos,
} from "./lib/habitos.js?v=2.37.0";
import {
  carregarPerfil,
  marcarPerfilInicializado,
  perfilInicializado,
  salvarPerfil,
} from "./lib/perfil.js?v=2.37.0";
import {
  correspondePreset,
...
} from "./lib/rotina-preset.js?v=2.37.0";
import {
  ...
  PLANO_B_APRENDER,
  textoSugereAprender,
} from "./lib/aprender.js?v=2.37.0";
import {
  carregarEstudo,
  resetSessaoSeNovoDia,
  salvarEstudo,
} from "./lib/estudo-hub.js?v=2.37.0";
import { iniciarVozes } from "./lib/voz-sintese.js?v=2.37.0";
import {
  atualizarResultadoLivros,
  ligarPainelEstudo,
  renderPainelEstudo,
  renderResumoHoje,
} from "./lib/estudo-ui.js?v=2.37.0";
import {
  montarOpcoesCheguei,
  renderChegueiFeito,
  renderChegueiInicio,
  renderChegueiOpcoes,
} from "./lib/cheguei.js?v=2.37.0";
import {
  aplicarSugestaoIa,
  montarPayloadContextoIa,
  pedirOpcoesContexto,
} from "./lib/contexto-ia.js?v=2.37.0";
import { faixaDoDia } from "./lib/contexto-tempo.js?v=2.37.0";
import { registrarConclusao, resumoPadroesParaIA, alertaPreditivo } from "./lib/padroes.js?v=2.37.0";
import {
  mensagemStreakGlobal,
  mensagemStreakHabito,
  rotuloStreakInsights,
} from "./lib/streak-gentil.js?v=2.37.0";
import { modoBarulho, definirModoBarulho, filtrarModoBarulho } from "./lib/modo-barulho.js?v=2.37.0";
import { detectarTransicao, mensagemTransicao } from "./lib/transicao-coach.js?v=2.37.0";
import { aplicarPreparoNoite, textoPreparoManha } from "./lib/preparar-amanha.js?v=2.37.0";
import { interpretarFala, rotuloContextoVoz } from "./lib/voz-contexto.js?v=2.37.0";
import { pedirResumoDiario, pedirPlanoSemana, pedirCaminhoNorth } from "./lib/ia-servicos.js?v=2.37.0";
import {
...
} from "./lib/estudo-parceiro.js?v=2.37.0";
import { escutarDictado, pararEscuta, suportaReconhecimentoVoz } from "./lib/estudo-fala.js?v=2.37.0";
import {
  arquivarVersaoNota,
  ...
  rotuloMotivoVersao,
} from "./lib/diario-historico.js?v=2.37.0";
import {
  aplicarExplicacoesNeuro,
  carregarExplicacoesNeuro,
  mesclarExplicacoesNeuro,
} from "./lib/neuro-explicar.js?v=2.37.0";
import { sondarIaNeuro, iaNeuroDisponivel } from "./lib/neuro-ia.js?v=2.37.0";
import {
  marcarGuiaVisto,
  marcarPassoGuia,
  guiaJaVisto,
  ...
  ROTEIRO_DEMO,
} from "./lib/guia-app.js?v=2.37.0";
import { obterDepois } from "./lib/mindos-hoje.js?v=2.37.0";
import { carregarEstadoMental, limparEstadoMental, salvarEstadoMental } from "./lib/mindos-estado.js?v=2.37.0";
import {
  htmlMemoriaHub,
  htmlNorthFocus,
  htmlNorthHome,
  parseMinutos,
} from "./lib/north-home.js?v=2.37.0";
import { htmlMindosRotina } from "./lib/mindos-rotina.js?v=2.37.0";
import { htmlMindosEmocional } from "./lib/mindos-emocional.js?v=2.37.0";
import { registrarPraticaEmocional, resumoEmocional } from "./lib/emocional-dados.js?v=2.37.0";
import { calcularCaminhoNorth } from "./lib/north-caminho.js?v=2.37.0";
import {
  ...
  resumoAprende,
} from "./lib/north-aprende.js?v=2.37.0";
import { htmlMindosSemana } from "./lib/mindos-semana.js?v=2.37.0";
import { htmlMindosInsights } from "./lib/mindos-insights.js?v=2.37.0";
import { htmlMindosEstudo } from "./lib/mindos-estudo.js?v=2.37.0";
import {
  carregarCognitivo,
...
} from "./lib/cognitivo-dados.js?v=2.37.0";
import {
  htmlCognitivoAgora,
...
} from "./lib/cognitivo-ui.js?v=2.37.0";
import { escolherConviteNorth } from "./lib/north-convite.js?v=2.37.0";
import { aplicarIdiomaHtml, definirCallbackIdioma, localeTag, setLocale, t } from "./lib/i18n.js?v=2.37.0";
import {
...
} from "./lib/migracao-host.js?v=2.37.0";
import {
  ehHorarioDificil,
  mensagemTarde,
  sugestaoTarde,
} from "./lib/tarde.js?v=2.37.0";
import {
  complementoCoachDiario,
  gerarResumoSemana,
  sugerirHabito,
  textoSugestao,
} from "./lib/inteligencia.js?v=2.37.0";
import {
  ...
  verificarLembretes,
} from "./lib/lembretes.js?v=2.37.0";
import { sincronizarAgendaSW } from "./lib/agenda-notif.js?v=2.37.0";
import {
  cancelarTimer,
  cronometroAtivo,
  formatarTimer,
  ...
  textoCountdown,
  timerAtivo,
} from "./lib/foco.js?v=2.37.0";
import {
  carregarPerfilRotina,
  gerarRotina,
  salvarPerfilRotina,
} from "./lib/rotina-local.js?v=2.37.0";
import {
  adicionarInbox,
  alternarPrioridade,
  aplicarLimiteDiario,
...
const CHAVE_CAMINHO_IA = "north-caminho-ia-v1";

function chaveCaminhoIa(caminho) {
  const estado = carregarEstadoMental() || "";
  const faixa = caminho?.faixa || "";
  return `${hojeStr()}|${estado}|${faixa}|${caminho?.tipo || ""}`;
}

function lerCacheCaminhoIa(chave) {
  try {
    const mapa = JSON.parse(localStorage.getItem(CHAVE_CAMINHO_IA) || "{}");
...
function salvarCacheCaminhoIa(chave, textos) {
  try {
    const mapa = JSON.parse(localStorage.getItem(CHAVE_CAMINHO_IA) || "{}");
    mapa[chave] = textos;
    const chaves = Object.keys(mapa);
    if (chaves.length > 14) {
      for (const k of chaves.slice(0, chaves.length - 14)) delete mapa[k];
    }
    localStorage.setItem(CHAVE_CAMINHO_IA, JSON.stringify(mapa));
  } catch {
    /* ignore */
  }
}

function aplicarTextosCaminhoIa(textos) {
  if (!mindosRoot || !textos) return;
  const titulo = mindosRoot.querySelector(".north-caminho-titulo");
  const mensagem = mindosRoot.querySelector(".north-caminho-mensagem");
  const convite = mindosRoot.querySelector(".north-caminho-convite");
  if (titulo && textos.titulo) titulo.textContent = textos.titulo;
  if (mensagem && textos.mensagem) mensagem.textContent = textos.mensagem;
  if (convite && textos.convite) convite.textContent = textos.convite;
}

async function enriquecerCaminhoComIa(caminho, token) {
  if (!caminho?.tipo) return;
  if (!carregarEstadoMental()) return;

  const chave = chaveCaminhoIa(caminho);
  const cache = lerCacheCaminhoIa(chave);
  if (cache?.titulo) {
    if (token !== northCaminhoIaToken) return;
    aplicarTextosCaminhoIa(cache);
    return;
  }

  const perfil = carregarPerfil();
  const resp = await pedirCaminhoNorth({
    estado: carregarEstadoMental(),
    faixa: caminho.faixa,
    tipo: caminho.tipo,
    prioridades: perfil.prioridadesVida || [],
    resumoAprende: resumoAprende(),
    resumoEmocional: resumoEmocional(),
    atual: {
      titulo: caminho.titulo,
      mensagem: caminho.mensagem,
      convite: caminho.convite,
    },
  });

  if (token !== northCaminhoIaToken) return;
  if (!resp?.ok || !resp.titulo) return;

  const textos = {
    titulo: resp.titulo,
    mensagem: resp.mensagem,
    convite: resp.convite,
  };
  salvarCacheCaminhoIa(chave, textos);
  aplicarTextosCaminhoIa(textos);
}
...
function desenharMindosHoje() {
  ...
  northCaminhoIaToken += 1;
  const token = northCaminhoIaToken;
  enriquecerCaminhoComIa(caminho, token);
}
